...
def create_builder_task(agent: Agent) -> Task:
    return Task(
        description=(
            ...
            "1. Extract the TSD object from the deal. Prioritise MVP features (phase_1) "
            "first — build the core 20% that delivers 80% of the value before extras.\n"
            "2. Call the build_and_deploy tool. CRITICAL: Pass the TSD as a JSON string "
            "to the parameter tsd_json like this:\n"
            "   build_and_deploy(tsd_json=json.dumps(tsd_object))\n"
            "   Where tsd_object is the TSD dictionary from the deal.\n"
